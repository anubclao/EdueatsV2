...
export const MenuPlanner = () => {
  ...
  const saveMenu = () => {
    ...
    const existingMenu = db.getDailyMenu(selectedDate);
    if (existingMenu?.isPublished) {
      alert("El menú para esta fecha ya ha sido publicado y no puede ser publicado de nuevo.");
      return;
    }

    // Validate if at least one 'main' or 'plato fuerte' recipe is selected
    const mainCategoryIds = categories.filter(cat => cat.name.toLowerCase() === 'main' || cat.name.toLowerCase() === 'plato fuerte').map(cat => cat.id);
    const hasMainDish = menuItems.some(item => {
      const recipe = recipes.find(r => r.id === item.recipeId);
      return recipe && mainCategoryIds.includes(recipe.category);
    });

    if (!hasMainDish) {
      alert("Debes seleccionar al menos un plato fuerte ('main' o 'plato fuerte') antes de publicar el menú.");
      return;
    }
  ...
  const isMenuPublishedForSelectedDate = db.getDailyMenu(selectedDate)?.isPublished || false;
