...
export const GlobalVariables = () => {
  ...
  const fetchVariables = () => {
    setVariables(db.getGlobalVariables());
  };
  ...
  const handleDelete = (id: string) => {
    if (confirm('¿Estás seguro de que deseas eliminar esta variable global?')) {
      const res = db.deleteGlobalVariable(id);
      if (res.success) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (exists) {
      // Update existing
      const res = db.updateGlobalVariable(formData);
      if (res.success) {
      ...
      // Add new
      const res = db.addGlobalVariable(formData);
      if (res.success) {
