...
export const dbService = {
  ...
  async getPreference(studentId: string, dayOfWeek: number): Promise<RecurringPreference | null> {
    const { data, error } = await supabase.from('recurring_preferences').select('*, recurring_preference_items(category, recipe_id)').eq('student_id', studentId).eq('day_of_week', dayOfWeek).single();
    return handleResponse({ data, error });
  },
