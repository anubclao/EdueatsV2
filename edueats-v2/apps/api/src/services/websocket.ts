 ...
 */
async function canAccessOrder(userId: string, role: string, orderId: string): Promise<boolean> {
  if (role === 'admin') return true;
  if (!/^[a-z0-9-]{1,64}$/i.test(orderId)) return false; // basic format guard
  const [rows] = await pool.execute(
    'SELECT 1 FROM orders WHERE id = ? AND student_id = ? LIMIT 1',
    [orderId, userId],
  ) as any[];
...
export function initWebSocket(httpServer: HTTPServer) {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('join-order', async (orderId: string, ack?: (res: { ok: boolean; error?: string }) => void) => {
      try {
    ...
    socket.on('join-admin', (ack?: (res: { ok: boolean; error?: string }) => void) => {
      if (role !== 'admin') {
        ack?.({ ok: false, error: 'forbidden' });
        return;
      }
