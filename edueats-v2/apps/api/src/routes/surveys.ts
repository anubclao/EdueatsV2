...
// --- Definitions ---
surveysRouter.get('/definitions', requireAuth, async (req, res) => {
  const schoolId = getSchoolId(req);
  try {
    const [rows] = await pool.query(
      "SELECT id, title, DATE_FORMAT(start_date, '%Y-%m-%d') as startDate, DATE_FORMAT(end_date, '%Y-%m-%d') as endDate, is_active as isActive, created_at as createdAt FROM survey_definitions WHERE school_id = ? ORDER BY start_date DESC",
      [schoolId]
    ) as any[];
...
surveysRouter.post('/definitions', requireAuth, requireRoles('admin'), async (req, res) => {
  ...
  try {
    ...
    await pool.execute(
      'INSERT INTO survey_definitions (id, title, start_date, end_date, is_active, created_at, school_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [id, title, startDate, endDate, isActive ? 1 : 0, created, schoolId]
    );
...
surveysRouter.put('/definitions/:id', requireAuth, requireRoles('admin'), async (req, res) => {
  ...
  try {
    await pool.execute(
      'UPDATE survey_definitions SET title=?, start_date=?, end_date=?, is_active=? WHERE id=? AND school_id=?',
      [title, startDate, endDate, isActive ? 1 : 0, req.params.id, schoolId]
    );
    res.json({ success: true });
  } catch (e: any) { res.status(500).json({ error: 'Error interno del servidor.' }); }
});
...
surveysRouter.delete('/definitions/:id', requireAuth, requireRoles('admin'), async (req, res) => {
  const schoolId = getSchoolId(req);
  try {
    await pool.execute('DELETE FROM survey_definitions WHERE id=? AND school_id=?', [req.params.id, schoolId]);
    res.json({ success: true });
  } catch (e: any) { res.status(500).json({ error: 'Error interno del servidor.' }); }
});
...
surveysRouter.get('/results/check', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const [rows] = await pool.execute(
      ...
      [userId, surveyDefId, getSchoolId(req)]
    ) as any[];
...
surveysRouter.get('/results', requireAuth, async (req, res) => {
  ...
  try {
    let sql = `SELECT id, survey_definition_id as surveyDefinitionId, user_id as userId,
      ...
      FROM survey_results WHERE school_id = ?`;
...
surveysRouter.post('/results', requireAuth, async (req, res) => {
  const schoolId = getSchoolId(req);
...
surveysRouter.put('/results/:id', requireAuth, requireRoles('admin'), async (req, res) => {
  ...
  try {
    await pool.execute('UPDATE survey_results SET admin_response=?, status=? WHERE id=? AND school_id=?',
      [adminResponse ?? null, status, req.params.id, schoolId]);
    res.json({ success: true });
  } catch (e: any) { res.status(500).json({ error: 'Error interno del servidor.' }); }
});
