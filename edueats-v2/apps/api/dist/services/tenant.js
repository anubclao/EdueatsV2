 ...
 */
export function scopedWhere(table, existingWhere = '') {
    if (!existingWhere)
        return `WHERE school_id = ?`;
 ...
 */
export function andScoped(table = '') {
    return `AND school_id = ?`;
}
/**
 * Resuelve el schoolId de un request. Falla ruidosamente si no existe.
 */
export function getSchoolId(req) {
    const sid = req.schoolId;
    if (!sid) {
        throw new Error(`[tenant] schoolId no presente en el request. ` +
